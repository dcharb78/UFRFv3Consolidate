...
from src.ym.torus_analysis import eigenphases, cartan_coords, chord_scores, rayleigh_R
from src.ym.su2_subgroups import project_su2
from src.ym.wloop_fft2d import fft2d_power
from src.ym.characters import trace_fundamental, trace_square, polyakov_eigenphases

def build_links(N=16, period=13, eps=0.2, seed=1):
    rng = np.random.default_rng(seed)
    # simple near-identity links by exponentiating random Hermitian (not full SU(3) exp for brevity)
...
def run_battery(N=16, Lmax=64, periods=(13,26), eps=0.2, seed=11):
    ...
    for p in periods:
        ...
        P = fft2d_power(W)
        # eigenphases/chords from a representative set of loops
        angs=[]; chords=[]
        for L in (8,16,32,48):  # sample a few sizes
            U = wilson_loop(Ux,Uy,0,0,min(L,Lmax))
            ang = eigenphases(U); angs.append(ang.tolist())
...
if __name__=='__main__':
    ...
    with open("artifacts/su3_multidim.json","w") as f:
